...
class Assembler {
    ...
    lines.forEach((line, lineNo) => {
      if (line.length == 0) {
        ...
        let lineWithoutComment = line
          .replace(Assembler._regexComment, "")
          .trim();
      ...
      for (let instruction of Assembler._langDefinition!.instructions) {
        ...
        for (let instructionRegex of instruction.regexes) {
          ...
          if (result != null) {
            ...
            let encodedResults: string[] = this.encodeInstruction(result, lineNo);
   ...
   */
  private encodeInstruction(regexMatchArrayInstruction: RegExpMatchArray, line: number): string[] {
    ...
    if (regexMatchArrayInstruction.length > 3) {
      ...
      encodedOperandValue2 = this.encodeOperandValue(regexMatchArrayInstruction[3], line);
    } else {
      ...
      encodedOperandValue2 = this.encodeOperandValue("", line);
    }
    ...
    if (regexMatchArrayInstruction.length > 2) {
      ...
      encodedOperandValue1 = this.encodeOperandValue(regexMatchArrayInstruction[2], line);
    } else {
      // No operand given
      addressingModeOperand1 = this.encodeOperandAddressingMode("", line);
      typeOperand1 = this.encodeOperandType("", line);
      encodedOperandValue1 = this.encodeOperandValue("", line);
    }
   ...
   */
  private encodeOperandValue(operand: string, line: number): string {
  ...
  private encodeOperandType(operand: string, line: number): string {
    if (operand.length === 0) {
      return "0000000".padStart(Assembler.WORD_WIDTH, "0");
    }

    if (operand.startsWith("*%") || operand.startsWith("%")) {
      return "11000000".padStart(Assembler.WORD_WIDTH, "0");
    }

    if (operand.startsWith("$")) {
      return "10100000".padStart(Assembler.WORD_WIDTH, "0");
    }

    if (operand.startsWith("@")) {
      return "1110000".padStart(Assembler.WORD_WIDTH, "0");
    }
  ...
  public async loadLanguageDefinition(f: File): Promise<void> {
    ...
    Assembler._langDefinition = JSON.parse(data);
    return;
  }
   ...
   */
  public compile(f: File): string[] {
    const data: string = readFileSync(f.path).toString();
    const lines: Map<number, string> = this.preprocess(data);
