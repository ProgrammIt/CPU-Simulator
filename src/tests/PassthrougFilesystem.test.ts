...
describe('PassthroughFilesystem', () => {
    ...
    describe('IO operations', () => {
        ...
        test('io_seek adjusts position correctly', () => {
            ...
            expect(buffer.getUint8(1)).toBe(101);
            expect(buffer.getUint8(2)).toBe(102);   
            fs.io_close(fd);
        });
        ...
        test('io_read_buffer reads data correctly', () => {
            ...
            expect(buffer.getUint8(1)).toBe(2);
            expect(buffer.getUint8(2)).toBe(3);            
            fs.io_close(fd);
        });
